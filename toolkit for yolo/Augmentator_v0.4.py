...
version: 2025-11-12 v0.4

put this script in the root directory of the dataset, and run it.

usage1: python Augmentator_v0.4.py --format keypoint
    this 



usage2: python Augmentator_v0.4.py --mode process --format segment

* support for YOLOv12 segment format
* support for all transforms: cw, ccw, hflip, vflip
* support for test mode to preview the results
* support for process mode to process the dataset
* support for saving the results to new files
* support for saving the results to new files
...
def transform_label_line_seg(line, transform):
    """
    將一行 YOLOv12 segment 標籤資料進行變換，
    格式：class cx cy w h mask1_x mask1_y mask1_v mask2_x mask2_y mask2_v ...
    """
    ...
    num_kpts = (len(parts) - 5) // 2
    for i in range(num_kpts):
        kp_x = float(parts[5 + 2*i])
        kp_y = float(parts[5 + 2*i + 1])
...
# ---------------------------
def draw_annotations(img, label_lines, format, image_size=640):
    """
...
# 資料集處理主程式
# ---------------------------
def process_dataset(base_dirs, transforms_to_apply, format, test_mode=False):
    """
    ...
    if test_mode:
        ...
        for t in transforms_to_apply:
            ...
            if format == "keypoint":
                ...
                t_labels = [transform_label_line_seg(line, t) for line in label_lines]
            else:
        ...
        # 繪製標註
        annotated_imgs = [draw_annotations(im, labs, format) for im, labs in zip(transformed_imgs, transformed_labels)]
...
def main():
    ...
    parser.add_argument("--format", choices=["keypoint", "segment"], default="keypoint",
                        help="標籤格式：keypoint 或 segment")
    args = parser.parse_args()
    transforms_to_apply = [t.strip() for t in args.transforms.split(",") if t.strip()]

    base_dirs = ["train", "test", "valid"]
    process_dataset(base_dirs, transforms_to_apply, args.format, test_mode=(args.mode=="test"))
